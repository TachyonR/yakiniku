...
Page({
  ...
  bindGetJob: function () {
    ...
    .update({
      data: {
        thumbnail: this.data.myNickname
      }
    })
    .then(res => {
      this.updateLists()
  ...
  bindToGreen: function () {
    var _task = this.data.detailedTask
    _task.thumbnailStatus = true
    _task.accomplished = this.checkTask(_task)
    tasks
    .doc(this.data.detailedTask._id)
    .update({
      data: {
